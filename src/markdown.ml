...
let infer_backquotes_needed min_allowed c =
...
let rec inline ~prefix buf = function
  ...
  | Strong (_, il) ->
      ...
      Buffer.add_string buf "**"
  | Code (attr, c) ->
      let n = infer_backquotes_needed 1 c in
      nchar n buf '`';
      if c.[0] = '`' then sp buf;
      Buffer.add_string buf c;
      if c.[String.length c - 1] = '`' then sp buf;
      nchar n buf '`';
...
and add_code_block ~first_item_prefix ~prefix buf attr label c =
  let n = infer_backquotes_needed 3 c in
  nchar n buf '`';
